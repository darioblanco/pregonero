...
pub struct Client<T> {
    transport: Framed<T, codec::ImapCodec>,
    state: State,
    request_ids: IdGenerator,
}

impl TlsClient {
    pub async fn connect(server: &str) -> io::Result<(codec::ResponseData, Self)> {
        ...
        let stream = connector
            ...
            .await?;
        let mut transport = codec::ImapCodec::default().framed(stream);
...
{
    type Item = Result<codec::ResponseData, io::Error>;
